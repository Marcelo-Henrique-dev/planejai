...
const statusStyles = {
  viable: {
    ...
    className: 'bg-green-100 text-green-700 dark:bg-green-300/30 dark:text-green-400',
  },
  needs_adjustment: {
    ...
    <div className="lg:scrollbar-thin lg:max-h-93 lg:overflow-y-auto lg:pr-2 lg:[scrollbar-color:var(--border)_transparent]">
      <section className="flex flex-col gap-2">
        <div className="flex flex-col items-start gap-2 sm:flex-row">
          ...
          {status && (
            <span className={`w-fit rounded-full px-2.5 text-xs font-semibold ${status.className}`}>
              {status.label}
            </span>
          )}
        </div>
